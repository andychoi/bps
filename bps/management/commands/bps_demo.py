        ...
        for year in years:
            for version in Version.objects.all():
                ...
                for org in OrgUnit.objects.all():
                        ...
                        for svc in services:
                            for period in Period.objects.all():
                                ...
                                PlanningFact.objects.update_or_create(
                                    ...
                                    driver_refs={'Type':'Actual'},key_figure=KeyFigure.objects.get(code='FTE'),
                                    defaults={'value':round(random.uniform(1,8),2),'uom':hrs,'ref_value':0,'ref_uom':None}
                                )
                                # COST actual
                                PlanningFact.objects.update_or_create(
                                    request=dr,session=ps,version=version,year=year,period=period,org_unit=org,service=svc,account=None,
                                    driver_refs={'Type':'Actual'},key_figure=KeyFigure.objects.get(code='COST'),
                        ...
                        for period in Period.objects.all():
                            for scen,vcode in SCENARIOS.items():
                                ...
                                PlanningFact.objects.update_or_create(
                                    ...
                                    driver_refs={'Scenario':scen},key_figure=KeyFigure.objects.get(code='FTE'),
                                    defaults={'value':round(random.uniform(1,8),2),'uom':hrs,'ref_value':0,'ref_uom':None}
                                )
                                for country in OUTSOURCING_COUNTRIES:
                                    PlanningFact.objects.update_or_create(
                                        request=dr,session=ps,version=ver,year=year,period=period,org_unit=org,service=svc,account=None,
                                        driver_refs={'Scenario':scen,'Type':'Outsourced','Country':country},key_figure=KeyFigure.objects.get(code='FTE'),
                                        defaults={'value':round(random.uniform(0,2),2),'uom':hrs,'ref_value':0,'ref_uom':None}
                                    )
                            # labor cost
                            PlanningFact.objects.update_or_create(
                                request=dr,session=ps,version=ver,year=year,period=period,org_unit=org,service=svc,account=None,driver_refs={},
                            ...
                            PlanningFact.objects.update_or_create(
                                request=dr,session=ps,version=ver,year=year,period=period,org_unit=org,service=svc,account=None,
                                driver_refs={'Driver':random.choice(['UserCount','ServerCount'])},key_figure=KeyFigure.objects.get(code='LICENSE'),
                    ...
                    PlanningFact.objects.update_or_create(request=dr,session=ps,version=Version.objects.get(code='DRAFT'),year=year,period=first_period,org_unit=org,service=None,account=None,driver_refs={'Type':'Admin'},key_figure=KeyFigure.objects.get(code='ADMIN'),defaults={'value':round(random.uniform(1000,5000),2),'uom':usd,'ref_value':0,'ref_uom':None})
                    PlanningFact.objects.update_or_create(request=dr,session=ps,version=Version.objects.get(code='DRAFT'),year=year,period=first_period,org_unit=org,service=None,account=None,driver_refs={'Type':'OrgMgmtCost'},key_figure=KeyFigure.objects.get(code='COST'),defaults={'value':round(random.uniform(5000,15000),2),'uom':usd,'ref_value':0,'ref_uom':None})
                    if shared_service:
                        for cbu in CBU.objects.filter(is_active=True):
                            PlanningFact.objects.update_or_create(request=dr,session=ps,version=Version.objects.get(code='DRAFT'),year=year,period=first_period,org_unit=org,service=shared_service,account=None,driver_refs={'Type':'SharedService','CBU':cbu.code},key_figure=KeyFigure.objects.get(code='COST'),defaults={'value':round(random.uniform(2000,8000),2),'uom':usd,'ref_value':0,'ref_uom':None})
